...
fit <- survfit(Surv(survival, delic_binary) ~ n_borrowers, data = data)

ggsurvplot(fit, 
           data=data,
           palette=c(basem1, basem2, "black"),
           conf.int = TRUE,
           pval = TRUE)


fit <- survfit(Surv(survival, delic_binary) ~ state, data = data %>% filter(state %in% c("CA", "TX", "FL", "NY", "PA")))
...
fit <- survfit(Surv(survival, delic_binary) ~ dataset, data = data)
...
ggsave(p_pca, filename = "plots/pca.pdf", width = 12, height = 5,  dpi = 300)
```
