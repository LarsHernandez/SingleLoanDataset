...
data_model <- subset(data, !is.na(data$delic_binary6)) %>% 
  select(delic_binary6, credit_score, new_homeowner, state, channel, loan_purpose, debt_to_income, property_type, upb, seller, rate, units, ocu_status, loan_to_value, n_borrowers, White, median_income2018) %>% 
  #mutate(delic_binary6 = ifelse(delic_binary6==TRUE,"yes","no")) %>% 
  mutate(delic_binary6 = as.factor(delic_binary6)) %>%
...
index    <- createDataPartition(data_model$delic_binary6, p = 0.90, list = FALSE)
...
cv <- trainControl(method = "cv", number = 5) # , classProbs = TRUE , summaryFunction = twoClassSummary , sampling = "rose"

model_recipe_steps <- recipe(delic_binary6 ~ ., data = training) %>%
...
#training  <- ROSE(delic_binary6 ~ ., data = training, p = 0.5, seed = 1)$data

#predict -----------------------------------------------------------------

t0 <- Sys.time()
#fit_tre <- train(delic_binary6 ~ .,
#                 data      = training,
#                 trControl = cv, 
#                 method    = "rpart", 
#                 metric    = "Kappa")
...
#                 metric    = "Kappa")
t4 <- Sys.time()
#fit_lda <- train(delic_binary6 ~ .,
#                 data      = training,
#                 trControl = cv, 
#                 method    = "lda",
#                 metric    = "Kappa")
t5 <- Sys.time()
fit_net <- train(delic_binary6 ~ .,
                 data      = training,
                 trControl = cv, 
                 method    = "glmnet",
                 metric    = "Kappa")
t6 <- Sys.time()
#fit_knn <- train(delic_binary6     ~ .,
#                 data      = training,
#                 trControl = cv, 
#                 method    = "kknn",
#                 metric    = "ROC")
t7 <- Sys.time()
fit_xgb <- train(delic_binary6     ~ .,
                 data      = training,
                 trControl = cv, 
                 method    = "xgbTree",
                 metric    = "Kappa")
t8 <- Sys.time()
#fit_svm <- train(delic_binary6     ~ .,
#   data      = training,
#   trControl = cv, 
#   method    = "svmLinear",
#   metric    = "Kappa")
...
net <- sspec(table(predict(fit_net,  newdata=test), test$delic_binary6),   "Elastic Net")
xgb <- sspec(table(predict(fit_xgb,  newdata=test), test$delic_binary6),   "Extreme Gradient Boosting")

df_del6 <- rbind(net,xgb) 

conf_del6<-rbind(table(predict(fit_net,  newdata=test), test$delic_binary6),
table(predict(fit_xgb,  newdata=test), test$delic_binary6))

ggplot(df_del6, aes(.metric, .estimate, fill = reorder(model, desc(.estimate)))) + 
